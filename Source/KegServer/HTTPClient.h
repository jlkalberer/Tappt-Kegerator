...
// FIXME Update tempToPachube example to calculate Content-Length correctly
    ...
    HttpClient(WiFly& aClient, const char* aProxy =NULL, uint16_t aProxyPort =0);
#else
    HttpClient(WiFly& aClient);
#endif
    ...
    */
	...
	virtual size_t write(const uint8_t *aBuffer, size_t aSize) { if (iState < eRequestSent) { finishHeaders(); }; Serial.write((char*)aBuffer); return iClient->write(aBuffer, aSize); };
    ...
    */
    ...
    virtual void stop();
	virtual uint8_t connected() { return true; /*iClient->available();*/ };
